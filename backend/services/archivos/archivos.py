...
archivos_bp = APIRouter(prefix="/v3/archivos", tags=["archivos"])
...
async def subir_archivo(archivo: UploadFile):
    try:
        ...
        # Insert into MongoDB
        archivos.insert_one(
            ArchivoNew(**archivo_res).model_dump()
        )  # Assuming ArchivoNew is a Pydantic model

        return JSONResponse(
            content={"mensaje": f"Archivo subido exitosamente", "url": url},
            status_code=201,
        )
