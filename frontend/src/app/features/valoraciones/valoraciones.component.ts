...
export class ValoracionesComponent implements OnInit {
  ...
  @Output() reputacionMediaCalculada: EventEmitter<number> =
  ...
  obtenerValoraciones(): void {
    if (this.idUsuario) {
      this.valoracionesService
        .getValoraciones({ idUsuarioValorado: this.idUsuario })
        .subscribe({
          next: (valoraciones) => {
  ...
  calcularReputacionMedia(): void {
    if (this.valoraciones.length > 0) {
      const totalNotas = this.valoraciones.reduce(
        (total, valoracion) => total + valoracion.nota,
        0,
      )
